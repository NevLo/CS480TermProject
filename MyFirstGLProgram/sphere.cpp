...
Sphere::Sphere(const char* fname, const char* nname) { // prec is precision, or number of slices
    ...
    if (m_texture)
        ...
        hasTexture = false;
    m_textureID = SOIL_load_OGL_texture(fname, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
    if (!m_textureID) {
    ...
    if (m_normaltexture)
        ...
        hasNormal = false;
    m_NormalTextureID = SOIL_load_OGL_texture(nname, SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y);
